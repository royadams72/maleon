...
exports.PageScrollInstance = undefined;

var _core = require('@angular/core');

var _ngxPageScrollConfig = require('./ngx-page-scroll-config');
...
var PageScrollInstance = function () {
    ...
    function PageScrollInstance(namespace, document) {
        ...
        this._namespace = _ngxPageScrollConfig.PageScrollConfig._defaultNamespace;
        /* Whether we scroll vertically (true) or horizontally (false) */
        this._verticalScrolling = _ngxPageScrollConfig.PageScrollConfig.defaultIsVerticalScrolling;
        /* Offset in px that the animation should stop above that target element */
        this._offset = _ngxPageScrollConfig.PageScrollConfig.defaultScrollOffset;
        /* Duration in milliseconds the scroll animation should last */
        this._duration = _ngxPageScrollConfig.PageScrollConfig.defaultDuration;
        /* Easing function to manipulate the scrollTop/scrollLeft value over time */
        this._easingLogic = _ngxPageScrollConfig.PageScrollConfig.defaultEasingLogic;
        /* Boolean whether the scroll animation should stop on user interruption or not */
        this._interruptible = _ngxPageScrollConfig.PageScrollConfig.defaultInterruptible;
        /* Whether the advanded offset calculation for inline scrolling should be used */
        this._advancedInlineOffsetCalculation = _ngxPageScrollConfig.PageScrollConfig.defaultAdvancedInlineOffsetCalculation;
        /* Event emitter to notify the world about the scrolling */
        this._pageScrollFinish = new _core.EventEmitter();
        /**
    ...
    PageScrollInstance.newInstance = function (options) {
        if (_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.namespace) || options.namespace.length <= 0) {
            options.namespace = _ngxPageScrollConfig.PageScrollConfig._defaultNamespace;
        }
        var pageScrollInstance = new PageScrollInstance(options.namespace, document);
        if (_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.scrollingViews) || options.scrollingViews.length === 0) {
        ...
        if (!_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.verticalScrolling)) {
            pageScrollInstance._verticalScrolling = options.verticalScrolling;
        }
        if (!_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.pageScrollOffset)) {
            pageScrollInstance._offset = options.pageScrollOffset;
        }
        if (!_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.pageScrollEasingLogic)) {
            pageScrollInstance._easingLogic = options.pageScrollEasingLogic;
        }
        if (_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.pageScrollDuration) && !_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.pageScrollSpeed)) {
        ...
        } else if (!_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.pageScrollDuration)) {
            pageScrollInstance._duration = options.pageScrollDuration;
        }
        if (!_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.pageScrollFinishListener)) {
            pageScrollInstance._pageScrollFinish = options.pageScrollFinishListener;
        }
        pageScrollInstance._interruptible = options.pageScrollInterruptible || _ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.pageScrollInterruptible) && _ngxPageScrollConfig.PageScrollConfig.defaultInterruptible;
        pageScrollInstance._advancedInlineOffsetCalculation = options.advancedInlineOffsetCalculation || _ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(options.advancedInlineOffsetCalculation) && _ngxPageScrollConfig.PageScrollConfig.defaultAdvancedInlineOffsetCalculation;
    ...
    PageScrollInstance.prototype.extractScrollTargetPosition = function () {
        ...
        if (this._isInlineScrolling) {
            ...
            if (this._advancedInlineOffsetCalculation && this.scrollingViews.length === 1) {
                ...
                while (!parentFound && !_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(parent_1)) {
                ...
                if (parentFound) {
                    ...
                    if (_ngxPageScrollConfig.PageScrollConfig._logLevel >= 2 || _ngxPageScrollConfig.PageScrollConfig._logLevel >= 1 && (0, _core.isDevMode)()) {
        ...
        return _ngxPageScrollUtil.PageScrollUtilService.extractElementPosition(this.document, scrollTargetElement);
    };
    /**
    ...
    PageScrollInstance.prototype.setScrollPosition = function (position) {
        var _this = this;
        if (_ngxPageScrollConfig.PageScrollConfig._logLevel >= 5 && (0, _core.isDevMode)()) {
        ...
        return this.scrollingViews.reduce(function (oneAlreadyWorked, scrollingView) {
            ...
            if (scrollingView && !_ngxPageScrollUtil.PageScrollUtilService.isUndefinedOrNull(startScrollPropertyValue)) {
                ...
                var isSmallMovement = scrollDistance < _ngxPageScrollConfig.PageScrollConfig._minScrollDistance;
    ...
    PageScrollInstance.prototype.attachInterruptListeners = function (interruptReporter) {
        ...
        _ngxPageScrollConfig.PageScrollConfig._interruptEvents.forEach(function (event) {
